...
const AuctionList = () => {
    ...
    const fetchAuctionData = async () => {
        try {
            ...
            const resolverResult = await provider.evaluateExpression('gno.land/r/varmeta/demo/v406/domain/registrar', `GetJoinedBid("${account.address}")`);
    ...
    const handleClaim = async (domain: string) => {
        try {
            ...
            const resolverResult = await provider.evaluateExpression('gno.land/r/varmeta/demo/v406/domain/registrar', `GetWinnerPrice("${domain}")`);
